...
int main ()
{
    ...
    while(t--){
        ...
        if(l == r){
            set<int> ss, ss1;
            for(int i = 0; i < l; i++)ss.insert(vecl[i]);
            for(int i = 0; i < r; i++)ss1.insert(vecr[i]);
            if(ss.size() < ss1.size()){
                swap(L, R);
                swap(l, r); //cout << 22 << endl;
                swap(vecl, vecr);
            }
        }
        for(int i = 0; i < l; i++){
            if(R.find(vecl[i]) != R.end()){
                auto it = L.find(vecl[i]);
                L.erase(it);
                auto it1 = R.find(vecl[i]);
                R.erase(it1);
            }
        }
