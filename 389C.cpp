...
int main ()
{
    ...
    int piles=1,pilesase=0,lastrem=0;
    for(int i=0;i<=100;i++){
        if(!A[i])continue;
        ll baki=A[i]-pilesase+lastrem;
        if(baki<=0){lastrem=0;continue;}
        piles+=(baki/(i+1));
        lastrem=(baki)%(i+1);
        if(lastrem!=0&&(baki/(i+1)>=1))piles++;
        pilsease=lastrem;
        //cout<<piles<<" "<<baki<<" "<<lastrem<<" "<<i<<" "<<A[i]<<endl;
    }
    cout<<piles<<endl;
